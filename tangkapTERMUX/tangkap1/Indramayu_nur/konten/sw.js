// sw.js otomatis untuk folder 'konten' — hanya cache isi folder ini
// DIBUAT OTOMATIS oleh buat_sw_folder.sh, jangan edit manual. Jalankan ulang script untuk update.
const CACHE_NAME='sw-konten-20260902';
...
self.addEventListener('activate', e=>{
  e.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('sw-konten')).filter(k=>k!==CACHE_NAME).map(k=>caches.delete(k)));
