// sw.js otomatis untuk folder 'kamera-hantu' — hanya cache isi folder ini
// DIBUAT OTOMATIS oleh buat_sw_folder.sh, jangan edit manual. Jalankan ulang script untuk update.
const CACHE_NAME='sw-kamera-hantu-20260902';
...
self.addEventListener('activate', e=>{
  e.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('sw-kamera-hantu')).filter(k=>k!==CACHE_NAME).map(k=>caches.delete(k)));
