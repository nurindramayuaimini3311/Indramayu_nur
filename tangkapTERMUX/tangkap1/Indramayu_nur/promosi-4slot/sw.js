// sw.js otomatis untuk folder 'promosi-4slot' — hanya cache isi folder ini
// DIBUAT OTOMATIS oleh buat_sw_folder.sh, jangan edit manual. Jalankan ulang script untuk update.
const CACHE_NAME='sw-promosi-4slot-20260902';
...
const CACHE_FILES=[
  ...
  './slot-4-club.html',
  './index.html',
  './template-komentar-properti.html',
];
...
self.addEventListener('activate', e=>{
  e.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('sw-promosi-4slot')).filter(k=>k!==CACHE_NAME).map(k=>caches.delete(k)));
